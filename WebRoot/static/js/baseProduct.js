...
function descartes(list){  
	...
	while(true){  
		...
		skuHtml=skuHtml+"<td><input type='text' class='text' name='prop_storageNum' maxlength='9' size='10' value='"+storageNum_val+"' onblur=\"skumap.put('storageNum"+sku_val+"',this.value);getSumSkuNum();\"><td><input type='text' class='text' maxlength='10' name='prop_originprice' size='10' value='"+originprice_val+"' onblur=\"skumap.put('originprice"+sku_val+"',this.value);\"></td><td><input name='prop_skuid' type='hidden' value='"+sku_val+"'><input name='prop_skuval' type='hidden' value=''><input type='text' class='text' maxlength='10' name='prop_price' size='10' value='"+price_val+"' onblur=\"skumap.put('price"+sku_val+"',this.value);\"></td></td><td hidden><input type='text' class='text' name='prop_merchantProdId' maxlength='13' size='10' value='"+merchantProdId_val+"' onblur=\"skumap.put('merchantProdId"+sku_val+"',this.value);\"></td><td><a href='javascript:void(0);' onclick='delPropCascade(this)'>Delete</a></td></tr>";
		//压入结果数组  
		result.push(temp);  
		temp = [];  
		//检查指针最大值问题  
...
$( function(){
	...
	if($("#prodBrandId").val()=="0"){
		$("#brandnamediv").html("<p><label>Brand Name:</label><input type='text' value='' name='prodBrandName' id='prodBrandName' class='text' maxlength='50' /></p>");
	}
...
function publishProduct(operation){
	...
	if(prodStorage==""){
		...
		if(prodStorage == '' && disabled == null){
			alertWarn("Pleanse input product stock!");
			$("#prodStorage").focus();
			return 0;
		}
    ...
    if(parseInt(prodOriginPrice)<parseInt(prodPrice)){
        alertWarn("Product Origin Price must be  great than or equal to Product Current Price!");
        $("#prodOriginPrice").focus();
        return 0;
    }
	var imgids="";
...
//		          +",,,"+$(this).find("td input[name=prop_price]").val()",,,"+$(this).find("td input[name=prop_merchantProdId]").val();
		...
		if(typeof($(this).find("input[name=prop_storageNum]").val()) != 'undefined'){
			...
			if(!reg.test(checkprice)){
				...
				ok = false;
				return false;
			}    if(parseInt(checkoriginprice)<parseInt(checkprice)){
                alertWarn("SKU origin price must be  great than or equal to SKU current price!");
                $("input[name=prop_price]").eq(i).focus();
                ok = false;
                return false;
            }
	...
	if(detailContent=="" || detailContent==null){
		alertWarn("Product detail description can not be empty!");
		$("#prodDetailDescContent").focus();
		return 0;
	}
    if(detailContent.length>50000){
        alertWarn("Product description is too long!");
        return 0;
    }
